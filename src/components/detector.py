...
class Detector(Entity):
    ...
    def attachBeamSplitter(self, name, timeline):
        for i in range(2):
...
class QSDetectorPolarization(QSDetector):
    ...
    def __init__(self, name: str, timeline: "Timeline", is_cow_protocol=False):
        super().__init__(name, timeline)

        # Initialize detectors with a condition to prevent infinite recursion
        for i in range(2):
            detector = Detector(name + ".detector" + str(i), timeline)
            self.detectors.append(detector)
            detector.attach(self)

        # Initialize the beamsplitter
        self.splitter = BeamSplitter(name + ".splitter", timeline, is_cow_protocol=is_cow_protocol)
        self.splitter.add_receiver(self.detectors[0])
        self.splitter.add_receiver(self.detectors[1])
    ...
    # For COW begin
    def getPhoton(self, photon=None, **kwargs) -> None:
        if self.get_generator().random() < self.efficiency:
            self.record_detection_cow(photon)

    def record_detection_cow(self, photon):
        # Notify observers about the detection event
        self.splitter.notify_observers(photon)
        self.notify_observers({'time': photon[1], 'photon': photon})

    def attach_observer(self, observer):
        if observer not in self.observers:
            self.observers.append(observer)
            # Also attach the observer to each detector in this polarization detector
            for detector in self.detectors:
                detector.attach_observer(observer)
