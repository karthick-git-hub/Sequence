...
from ..components.detector import QSDetector, QSDetectorPolarization, QSDetectorTimeBin, Detector
...
class Node(Entity):
    ...
    def send_qubit(self, dst: str, qubit) -> None:
        ...
        if isinstance(self.protocol_stack[0], COWProtocol):
            return self.qchannels[dst].transmit_cow_or_three_stage(qubit, "COW")
        elif isinstance(self.protocol_stack[0], ThreeStageProtocol):
            return self.qchannels[dst].transmit_cow_or_three_stage(qubit, "ThreeStage")
        else:
            return self.qchannels[dst].transmit(qubit, self)


    def receive_qubit(self, qubit_List) -> None:
        ...
        """
        if isinstance(qubit_List[0], tuple):
            ...
            self.components[self.first_component_name].get(qubit_List)
...
class QKDNode(Node):
    ...
    def __init__(self, name: str, timeline: "Timeline", encoding=polarization, stack_size=5,
        ...
        if encoding["name"] == "polarization":
            if stack_size > 2:
                qsdetector = Detector(qsd_name, timeline)
                qsdetector.attachBeamSplitter(qsd_name, timeline)
            else:
    ...
    def custom_get(self, photons: List["Photon"], **kwargs):
        qubit_list = []
